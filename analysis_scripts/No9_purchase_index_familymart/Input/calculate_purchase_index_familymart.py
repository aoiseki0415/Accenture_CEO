"""ファミリーマートの3健康ニーズについて、商品別・属性別の購買指数を算出する。

入力
----
1. このファイル内で用意する、ファミリーマート全体のDataFrame ``data1`` ～ ``data5``
2. No6のOutputに保存済みの商品別CSVと ``product_file_mapping.csv``
...
# No6で作成した商品別CSVの保存フォルダ名。
SOURCE_PROJECT_DIR_NAME = "No6_extract_confirmed_products_familymart"
...
def run(
    ...
    for health_need in HEALTH_NEEDS:
        ...
        if not product_source_dir.exists():
            raise FileNotFoundError(
                f"No6の健康ニーズ別フォルダが見つかりません: {product_source_dir}"
            )
        calculate_one_need(
