"""ファミリーマートの個別商品購買指数を、健康ニーズ×属性のセルへ集約する。

No9の健康ニーズ別フォルダから「商品別属性別_個別商品購買指数.csv」を読み、
...
出力
----
* ファミリーマート_対応商品群購買指数.csv
* ファミリーマート_平均使用商品数.csv
"""
...
SOURCE_PROJECT_DIR_NAME = "No9_purchase_index_familymart"
...
MAIN_OUTPUT_FILE_NAME = "ファミリーマート_対応商品群購買指数.csv"
COUNT_OUTPUT_FILE_NAME = "ファミリーマート_平均使用商品数.csv"
...
def validate_input_table(
    ...
    if table.empty:
        raise ValueError(
            f"入力表が空です。No9の出力を確認してください: {source_path}"
        )
...
def aggregate_one_need(
    ...
    if not source_path.exists():
        raise FileNotFoundError(f"No9の入力表が見つかりません: {source_path}")
...
def run() -> tuple[Path, Path]:
    ...
    if not source_output_dir.exists():
        raise FileNotFoundError(
            f"No9のOutputが見つかりません: {source_output_dir}"
        )
