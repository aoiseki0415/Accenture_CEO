...
SSDから復元したPythonコードのうち、最終的に実施した本分析の流れに対応する No4〜No13と、その後の原因分析用No14〜No21を保管しています。
...
| No21 | コンビニ3社・栄養バランス | 「対応」商品を同じ規則で一食完結型／食事補完型に分類し、一食完結型の商品数と割合を3社で比較。主分析はZaim確認済み商品のみ、全対応商品は補足分析として保存 | `No21_compare_response_product_roles/Input/compare_response_product_roles.py` |
...
11. No20：No17の二群について、惣菜類に占める一食完結型・食事補完型の購買記録数と構成割合を比較
12. No21：3社の栄養バランス対応商品に占める一食完結型の商品数と割合を、主分析・補足分析で比較
