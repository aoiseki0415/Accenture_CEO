"""セブンイレブンの3健康ニーズについて、商品別・属性別の購買指数を算出する。

入力
----
1. このファイル内で用意する、セブンイレブン全体のDataFrame ``data1`` ～ ``data5``
2. No4のOutputに保存済みの商品別CSVと ``product_file_mapping.csv``
...
def run(
    ...
    for health_need in HEALTH_NEEDS:
        ...
        if not product_source_dir.exists():
            raise FileNotFoundError(
                f"No4の健康ニーズ別フォルダが見つかりません: {product_source_dir}"
            )
        calculate_one_need(
